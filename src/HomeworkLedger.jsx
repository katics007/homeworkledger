...
import {
  ...
  RotateCcw, StickyNote, Video as VideoIcon
} from "lucide-react";
  ...
  return (
    <div className="hl-root">
      ...
      {booting ? (
        ...
        <StudentApp user={currentUser} api={api} onLogout={() => setCurrentUser(null)} />
      )}
...
// ---------- SHELL ----------
function Shell({ user, onLogout, tabs, active, setActive, roleLabel, api, children }) {
    ...
    <div className="hl-shell">
      <header className="hl-topbar">
        ...
        <div className="hl-topbar-right">
          ...
          <div className="hl-avatar sm">{user.name?.[0] ?? "?"}</div>
          <button className="hl-icon-btn" onClick={onLogout} title={t("logout")}><LogOut size={16} /></button>
        </div>
      </header>
  ...
  const addVerticalQuestion = () => {
    ...
    setQuestions((qs) => [
      ...
      { id: uid("q"), type: "vertical", a: Number(vA), b: Number(vB), op: vOp,
        text: `${vA} ${OP_SYMBOLS[vOp]} ${vB}`, answer, points: Number(vPts) || 1 },
    ]);
    setVA(""); setVB("");
  };

  const canSave = title.trim() && subject && className && questions.every((q) => q.text.trim() && q.answer.trim());

  const save = async () => {
    if (!canSave) { showToast(t("fillTitleErr"), "err"); return; }
    const entry = {
      id: uid("hw"), date, subject, className, title: title.trim(),
      questions: questions.map((q) => ({ ...q, points: Number(q.points) || 1 })),
      createdAt: Date.now(),
    };
    ...
    <div className="hl-page">
      ...
      <div className="hl-card">
        ...
        <div className="hl-q-list">
          ...
          {questions.map((q, i) => (
            <div className="hl-q-row" key={q.id}>
              ...
              <input className="hl-q-points" type="number" min="1" placeholder={t("pointsPlaceholder")} value={q.points} onChange={(e) => updateQuestion(q.id, "points", e.target.value)} />
        ...
        <div className="hl-vbuilder">
          ...
          <div className="hl-vbuilder-row">
            ...
            <input className="hl-vb-pts" type="number" min="1" placeholder={t("vPoints")} value={vPts} onChange={(e) => setVPts(e.target.value)} />
...
function ReviewScores({ api }) {
  const { homework, submissions, users, t, lang } = api;
  ...
  const maxScore = hw ? hw.questions.reduce((s, q) => s + (Number(q.points) || 1), 0) : 0;
          ...
          <>
            ...
            <div className="hl-table-wrap">
              <table className="hl-table">
                <thead><tr><th>{t("colStudent")}</th><th>{t("colUserId")}</th><th>{t("colStatus")}</th><th>{t("colScore")}</th></tr></thead>
                <tbody>
                  ...
                  {rows.length === 0 && <tr><td colSpan={4}><EmptyState text={t("noStudentsInClass")} /></td></tr>}
                </tbody>
              </table>
            </div>
          </>
...
function StudentApp({ user, api, onLogout }) {
  ...
  return (
    <Shell user={user} onLogout={onLogout} tabs={tabs} active={active} setActive={setActive} roleLabel={user.className} api={api}>
      {active === "today" && <DayHomework user={user} api={api} />}
      {active === "notes" && <NotesManager api={api} />}
...
function DayHomework({ user, api }) {
  ...
  const [sub_, setSub_] = useState("list"); // "list" | "scores" | "leaderboard"
  ...
  return (
    <div className="hl-page">
      ...
      <div className="hl-segmented">
        <button className={sub_ === "list" ? "active" : ""} onClick={() => setSub_("list")}><Calendar size={14} /> {t("subNavHomework")}</button>
        <button className={sub_ === "scores" ? "active" : ""} onClick={() => setSub_("scores")}><BarChart3 size={14} /> {t("subNavScores")}</button>
        <button className={sub_ === "leaderboard" ? "active" : ""} onClick={() => setSub_("leaderboard")}><Trophy size={14} /> {t("subNavLeaderboard")}</button>
      </div>

      {sub_ === "scores" && <MyScores user={user} api={api} embedded />}
      {sub_ === "leaderboard" && <Leaderboard api={api} highlightId={user.id} lockClass={user.className} embedded />}
        ...
        <>
          ...
          <div className="hl-hw-list">
            ...
            {forSelected.map((h) => {
              ...
              const maxScore = h.questions.reduce((s, q) => s + (Number(q.points) || 1), 0);
...
function SolveHomework({ hw, mode = "new", user, submissions, setSubmissions, reattempts, setReattempts, showToast, onDone, t, confirm }) {
  ...
  const submit = async () => {
    const unanswered = hw.questions.filter((q) => !normalize(answers[q.id]));
    if (unanswered.length > 0 && !(await confirm(t("blankConfirm", { n: unanswered.length })))) return;

    let score = 0;
    const maxScore = hw.questions.reduce((s, q) => s + (Number(q.points) || 1), 0);
    const detail = hw.questions.map((q) => {
      const correct = answersMatch(answers[q.id], q.answer);
      if (correct) score += Number(q.points) || 1;
      return { questionId: q.id, given: answers[q.id] || "", correct };
    });
        ...
        <div className="hl-result-card">
          ...
          <div className="hl-result-list">
            {hw.questions.map((q) => {
              ...
              return (
                <div className={`hl-result-row ${d.correct ? "ok" : "bad"}`} key={q.id}>
                  {d.correct ? <Check size={14} /> : <X size={14} />}
                  <span className="q">{q.text}</span>
                  <span className="a">{t("yourAnswer")} {d.given || "—"}{!d.correct && <> · {t("correctLabel")} {q.answer}</>}</span>
                </div>
              );
            })}
          </div>
...
function NotesManager({ api, isTeacher }) {
  const { notes, setNotes, showToast, t, confirm } = api;
  const [zoomNote, setZoomNote] = useState(null);
  ...
  const onUpload = async (e) => {
    ...
    try {
      ...
      const entry = {
        id: uid("note"), name: file.name,
        type: file.type === "application/pdf" ? "pdf" : "image",
        dataUrl, uploadedAt: Date.now(),
      };
      const ok = await setNotes([entry, ...notes]);
      if (ok) showToast(t("noteUploaded", { n: file.name }));
    } catch (err) {
      showToast(t("noteUploadErr"), "err");
    }
    setUploading(false);
  };

  const removeNote = async (n) => {
    if (!(await confirm(t("removeNoteConfirm", { n: n.name })))) return;
    await setNotes(notes.filter((x) => x.id !== n.id));
  };

  return (
    <div className="hl-page">
      <PageHead icon={<StickyNote size={20} />} title={t("notesTitle")} sub={isTeacher ? t("notesSubTeacher") : t("notesSub")} />

      {isTeacher && (
        <div className="hl-card">
          <label className="hl-btn primary hl-upload-btn">
            {uploading ? <Loader2 className="spin" size={15} /> : <Upload size={15} />} {t("uploadNote")}
            <input type="file" accept="image/*,application/pdf" onChange={onUpload} disabled={uploading} hidden />
          </label>
        </div>
      )}

      <div className="hl-notes-grid">
        {notes.length === 0 && <EmptyState text={t("noNotesYet")} />}
        {notes.map((n) => (
          <div className="hl-note-card" key={n.id}>
            <button type="button" className="hl-note-thumb" onClick={() => setZoomNote(n)}>
              {n.type === "image" ? <img src={n.dataUrl} alt={n.name} /> : <FileText size={34} />}
            </button>
            <div className="hl-note-name">{n.name}</div>
            {isTeacher && <button className="hl-icon-btn danger" onClick={() => removeNote(n)}><Trash2 size={14} /></button>}
          </div>
        ))}
      </div>

      {zoomNote && <NoteZoomModal note={zoomNote} onClose={() => setZoomNote(null)} t={t} />}
    </div>
  );
}

function NoteZoomModal({ note, onClose, t }) {
    ...
    <div className="hl-modal-overlay" onClick={onClose}>
      <div className="hl-zoom-card" onClick={(e) => e.stopPropagation()}>
        <div className="hl-zoom-toolbar">
          <span className="hl-note-name">{note.name}</span>
        ...
        <div className="hl-zoom-body">
          {note.type === "image" ? (
            <img src={note.dataUrl} alt={note.name} style={{ transform: `scale(${zoom})` }} />
          ) : (
            <iframe src={note.dataUrl} title={note.name} className="hl-pdf-frame" />
          )}
        </div>
      </div>
    </div>
...
function Style() {
  return (
    <style>{`
      ...
      .hl-login-card { width: 100%; max-width: 400px; background: var(--card); border: 1px solid var(--border); border-radius: 16px; padding: 24px; box-shadow: 0 10px 30px rgba(36,65,58,0.10); }
      ...
      .hl-brand-mark { width: 42px; height: 42px; border-radius: 11px; background: var(--board); color: #F3E7C9; display: flex; align-items: center; justify-content: center; flex-shrink: 0; }
      .hl-brand-title { font-family: 'Bitter', 'Noto Sans Devanagari', serif; font-weight: 800; font-size: 19px; color: var(--board-dark); letter-spacing: -0.2px; line-height: 1.2; }
      ...
      .hl-role-toggle button.active { background: var(--board); color: #F3E7C9; }
      ...
      .hl-btn.primary { background: var(--board); color: #F3E7C9; }
      .hl-btn.primary:hover { background: var(--board-dark); }
      ...
      .hl-demo-title { font-weight: 700; color: var(--board-dark); margin-bottom: 6px; font-family: 'Bitter', 'Noto Sans Devanagari', serif; }
      ...
      .hl-nav button.active { background: var(--board); color: #F3E7C9; }
      ...
      .hl-avatar { width: 32px; height: 32px; border-radius: 50%; background: var(--gold); color: #402D07; display: flex; align-items: center; justify-content: center; font-weight: 700; font-family: 'Bitter', 'Noto Sans Devanagari', serif; flex-shrink: 0; }
      ...
      .hl-page-icon { width: 38px; height: 38px; border-radius: 10px; background: var(--board); color: #F3E7C9; display: flex; align-items: center; justify-content: center; flex-shrink: 0; }
      .hl-page-head h2 { font-family: 'Bitter', 'Noto Sans Devanagari', serif; font-size: 20px; font-weight: 800; color: var(--board-dark); margin: 0 0 2px; line-height: 1.25; }
      .hl-page-head p { font-size: 13px; color: var(--ink-soft); margin: 0; line-height: 1.4; }
      .hl-subhead { font-family: 'Bitter', 'Noto Sans Devanagari', serif; font-size: 15px; font-weight: 700; color: var(--board-dark); margin: 24px 0 10px; }
      ...
      .hl-hw-date-day { font-family: 'Bitter', 'Noto Sans Devanagari', serif; font-weight: 800; font-size: 18px; color: var(--board-dark); line-height: 1; }
      .hl-hw-date-mon { font-size: 10px; color: var(--ink-soft); text-transform: uppercase; letter-spacing: 0.04em; }
      .hl-subject-badge { background: #EFE3C4; color: #6B4E14; font-size: 11px; font-weight: 700; padding: 5px 9px; border-radius: 7px; flex-shrink: 0; }
      ...
      .hl-stat { background: var(--card); border: 1px solid var(--border); border-radius: 12px; padding: 12px 16px; font-size: 11.5px; color: var(--ink-soft); font-weight: 600; min-width: 100px; flex: 1 1 100px; }
      .hl-stat span { display: block; font-family: 'Bitter', 'Noto Sans Devanagari', serif; font-size: 20px; font-weight: 800; color: var(--board-dark); }
      ...
      .hl-day-num { font-family: 'Bitter', 'Noto Sans Devanagari', serif; font-weight: 800; font-size: 15px; color: var(--ink); }
      .hl-day.today { border-color: var(--gold); }
      .hl-day.active { background: var(--board); }
      .hl-day.active .hl-day-name, .hl-day.active .hl-day-num { color: #F3E7C9; }
      .hl-day-dot { width: 5px; height: 5px; border-radius: 50%; background: var(--gold-dark); position: absolute; bottom: 4px; }
      .hl-day.active .hl-day-dot { background: #F3E7C9; }
      ...
      .hl-result-score { font-family: 'Bitter', 'Noto Sans Devanagari', serif; font-size: 32px; font-weight: 800; color: var(--board-dark); margin-top: 4px; }
      ...
      .hl-ledger-head, .hl-ledger-row { display: grid; grid-template-columns: 50px 1.4fr 1fr 56px 1.4fr; align-items: center; gap: 8px; padding: 11px 14px; }
      .hl-ledger-head { background: var(--paper); font-size: 10.5px; text-transform: uppercase; letter-spacing: 0.04em; color: var(--ink-soft); font-weight: 700; }
      .hl-ledger-row { border-top: 1px solid var(--border); font-size: 13.5px; }
      .hl-ledger-row.me { background: #FCF3DC; }
      .hl-rank { font-family: 'Bitter', 'Noto Sans Devanagari', serif; font-weight: 800; color: var(--board-dark); font-size: 15px; }
      ...
      .hl-saving { position: fixed; bottom: 16px; left: 16px; background: var(--board); color: #F3E7C9; font-size: 12px; font-weight: 600; padding: 8px 12px; border-radius: 20px; display: flex; align-items: center; gap: 6px; z-index: 50; }
      .hl-toast { position: fixed; bottom: 16px; right: 16px; background: var(--board); color: #F3E7C9; font-size: 13px; font-weight: 600; padding: 10px 16px; border-radius: 10px; display: flex; align-items: center; gap: 8px; z-index: 50; box-shadow: 0 8px 24px rgba(0,0,0,0.18); max-width: 320px; }
      ...
      .hl-modal-card { background: var(--card); border-radius: 16px; padding: 20px; width: 100%; max-width: 360px; box-shadow: 0 20px 50px rgba(0,0,0,0.25); }
      ...
      .hl-segmented button.active { background: var(--board); color: #F3E7C9; }
      ...
      .hl-vert-answer { width: 100%; text-align: right; border: 1px solid var(--border); background: var(--paper); border-radius: 8px; padding: 8px 10px; font-family: 'JetBrains Mono', monospace; font-size: 18px; font-weight: 700; min-height: 42px; }
      ...
      @media (max-width: 760px) {
        ...
        .hl-bottom-nav { display: flex; position: fixed; bottom: 0; left: 0; right: 0; background: var(--card); border-top: 1px solid var(--border); z-index: 30; padding: 4px 2px calc(4px + env(safe-area-inset-bottom)); box-shadow: 0 -6px 18px rgba(36,65,58,0.08); overflow-x: auto; }
        ...
        .hl-bottom-nav button.active { color: var(--board); background: #EFE3C4; }
