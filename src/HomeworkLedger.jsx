...
import {
  ...
  RotateCcw, Video as VideoIcon, Menu, Image as ImageIcon, ListChecks, Play
} from "lucide-react";
import { supabase } from "./supabaseClient";

// ---------- crash safety net ----------
// Without this, any unexpected render error (e.g. from a corrupted photo
// upload or a malformed realtime update) unmounts the whole React tree and
// the user just sees a blank white screen with no way to recover except a
// hard refresh. This catches that and shows a friendly recoverable message.
class ErrorBoundary extends React.Component {
  constructor(props) { super(props); this.state = { hasError: false }; }
  static getDerivedStateFromError() { return { hasError: true }; }
  componentDidCatch(error, info) { console.error("HomeworkLedger crashed:", error, info); }
  render() {
    if (this.state.hasError) {
      return (
        <div style={{
          minHeight: "100vh", display: "flex", flexDirection: "column", alignItems: "center",
          justifyContent: "center", gap: 14, padding: 24, textAlign: "center", fontFamily: "sans-serif",
        }}>
          <div style={{ fontSize: 17, fontWeight: 700 }}>Something went wrong.</div>
          <div style={{ fontSize: 14, color: "#666", maxWidth: 360 }}>
            Please reload the page. If this keeps happening after uploading a photo, try a smaller image.
          </div>
          <button
            onClick={() => window.location.reload()}
            style={{ padding: "10px 18px", borderRadius: 10, border: "none", background: "#111", color: "#fff", fontWeight: 700, cursor: "pointer" }}
          >
            Reload
          </button>
        </div>
...
const STRINGS = {
  mr: {
    ...
    fillTitleErr: "जतन करण्यापूर्वी किमान एक प्रश्न पूर्ण भरा.",
    ...
    vTabLabel: "उभा गणित", mcqTabLabel: "बहुपर्यायी", photoTabLabel: "फोटो",
    photoQTypeHint: "फक्त फोटो अपलोड करा — काहीही टाईप करण्याची गरज नाही.",
    addedQuestions: "जोडलेले प्रश्न", lbDaily: "आजचे", lbWeekly: "साप्ताहिक", lbMonthly: "मासिक",
    todaysLeaderboard: "आजची गुणवत्ता यादी",
    takePhoto: "फोटो काढा", photoQuestionLabel: "फोटो प्रश्न",
  },
  en: {
    ...
    fillTitleErr: "Fill in at least one complete question before saving.",
    ...
    vTabLabel: "Vertical Math", mcqTabLabel: "MCQ", photoTabLabel: "Photo",
    photoQTypeHint: "Just upload a photo — no typing needed.",
    addedQuestions: "Added questions", lbDaily: "Daily", lbWeekly: "Weekly", lbMonthly: "Monthly",
    todaysLeaderboard: "Today's leaderboard",
    takePhoto: "Take photo", photoQuestionLabel: "Photo question",
  },
};
function makeT(lang) {
  return (key, vars) => {
...
const toDevanagariDigits = (val) => String(val ?? "").replace(/[0-9]/g, (d) => DEVANAGARI_DIGITS[Number(d)]);
const toArabicDigits = (val) => String(val ?? "").replace(/[०-९]/g, (d) => String(DEVANAGARI_DIGITS.indexOf(d)));
// Vertical-math questions/answers are shown to students in Devanagari numerals;
// this renders any value (number or string) that way when the question type is "vertical".
const vertDisplay = (q, val) => (q && q.type === "vertical" ? toDevanagariDigits(val) : val);
...
const normalize = (s) => toArabicDigits(String(s ?? "")).trim().toLowerCase().replace(/\s+/g, " ");
      ...
      .on(
        ...
        (payload) => {
          try {
            ...
            if (setter && row.value !== undefined && row.value !== null) setter(row.value);
          } catch (e) { console.error("Ignoring malformed realtime update:", e); }
        }
      )
      .subscribe();
...
function SetHomework({ api }) {
  ...
  // Photo questions are image-only — no text/answer to type at all.
  const addPhotoQuestion = () => setQuestions((qs) => [...qs, { id: uid("q"), type: "photo", points: 1 }]);
  ...
  const canSave = subject && className && questions.length > 0 && questions.every((q) => {
    if (q.type === "mcq") return q.text.trim() && q.options.every((o) => o.trim()) && q.answer.trim();
    if (q.type === "photo") return !!q.image;
    return q.text.trim() && q.answer.trim();
  });

  // Students should see only one homework per subject per day: if the
  // teacher already set homework for this date + subject + class, the new
  // questions are appended to that same entry instead of creating another.
  const save = async () => {
    if (!canSave) { showToast(t("fillTitleErr"), "err"); return; }
    const newQuestions = questions.map((q) => ({ ...q, points: ptsOf(q) }));
    const existing = homework.find((h) => h.date === date && h.subject === subject && h.className === className);
    const nextHomework = existing
      ? homework.map((h) => (h.id === existing.id ? { ...h, questions: [...h.questions, ...newQuestions] } : h))
      : [{ id: uid("hw"), date, subject, className, title: subject, questions: newQuestions, createdAt: Date.now() }, ...homework];
    await setHomework(nextHomework);
    showToast(t("savedToast", { title: subject, cls: className, date: friendlyDate(date, lang) }));
    setQuestions([]);
  };
    ...
    <div className="hl-page">
      ...
      <div className="hl-card">
        ...
        <div className="hl-q-list">
          <div className="hl-q-head"><span>{t("questions")}</span></div>

          <div className="hl-segmented hl-qtype-tabs">
            <button type="button" className={qTab === "vertical" ? "active" : ""} onClick={() => setQTab("vertical")}>
              <ClipboardList size={15} /> {t("vTabLabel")}
            </button>
            <button type="button" className={qTab === "mcq" ? "active" : ""} onClick={() => setQTab("mcq")}>
              <ListChecks size={15} /> {t("mcqTabLabel")}
            </button>
            <button type="button" className={qTab === "photo" ? "active" : ""} onClick={() => setQTab("photo")}>
              <ImageIcon size={15} /> {t("photoTabLabel")}
            </button>
          </div>
          ...
          {qTab === "vertical" && (
            <div className="hl-vbuilder">
              ...
              {vA !== "" && vB !== "" && (
                <div className="hl-vb-preview mono">
                  {toDevanagariDigits(vA)} {OP_SYMBOLS[vOp]} {toDevanagariDigits(vB)} = {toDevanagariDigits(computeVertical(vA, vB, vOp))}
                </div>
              )}
            </div>
          )}
          ...
          {qTab === "mcq" && (
            <div className="hl-qtype-add">
              <button className="hl-btn ghost sm" type="button" onClick={addMcqQuestion}><Plus size={14} /> {t("addMcq")}</button>
            </div>
          )}

          {qTab === "photo" && (
            <div className="hl-qtype-add">
              <button className="hl-btn ghost sm" type="button" onClick={addPhotoQuestion}><Plus size={14} /> {t("addPhoto")}</button>
              <p className="hl-qtype-hint">{t("photoQTypeHint")}</p>
            </div>
          )}

          <button type="button" className="hl-qlist-toggle" onClick={() => setListOpen((v) => !v)}>
            {listOpen ? <ChevronLeft size={14} style={{ transform: "rotate(-90deg)" }} /> : <ChevronRight size={14} />}
            {t("addedQuestions")} ({questions.length})
          </button>

          {listOpen && questions.map((q, i) => (
              ...
              {q.type === "mcq" ? (
                <div className="hl-q-mcq">
                  <div className="hl-q-mcq-top">
                    ...
                    <button className="hl-icon-btn danger" onClick={() => removeQuestion(q.id)}><Trash2 size={15} /></button>
                  </div>
                ...
                <div className="hl-q-mcq">
                  <div className="hl-q-mcq-top">
                    <div className="hl-q-num">{i + 1}<ImageIcon size={11} className="hl-q-vert-mark" /></div>
                    <input className="hl-q-points" type="number" min="0" placeholder={t("pointsPlaceholder")} value={q.points} onChange={(e) => updateQuestion(q.id, "points", e.target.value)} />
                    <button className="hl-icon-btn danger" onClick={() => removeQuestion(q.id)}><Trash2 size={15} /></button>
                  </div>
        ...
        <button className="hl-btn primary" onClick={save} disabled={!canSave}><Save size={15} /> {t("saveHomework")}</button>
      </div>
        ...
        {grouped.map((h) => (
          <div className="hl-hw-item" key={h.id}>
            ...
            <div className="hl-hw-body">
              <div className="hl-hw-title">{h.subject}</div>
              <div className="hl-hw-meta">{h.className} · {t("questionsCount", { n: h.questions.length })}</div>
            </div>
            ...
            {hw.questions.map((q) => {
              ...
              return (
                <div className={`hl-result-row ${d.ungraded ? "" : d.correct ? "ok" : "bad"}`} key={q.id}>
                  ...
                  <div style={{ flex: 1 }}>
                    <div className="q">{q.type === "vertical" ? `${toDevanagariDigits(q.a)} ${OP_SYMBOLS[q.op]} ${toDevanagariDigits(q.b)}` : q.type === "photo" ? t("photoQuestionLabel") : q.text}</div>
                    <div className="a">
                      {d.given ? <>{t("yourAnswer")} {vertDisplay(q, d.given)}</> : null}
                      {!d.ungraded && !d.correct && <> · {t("correctLabel")} {vertDisplay(q, q.answer)}</>}
                    </div>
                    {q.type === "photo" && q.image && (
  ...
  if (solving) {
    return (
      <SolveHomework
        ...
        showToast={showToast} onDone={() => setSolving(null)} t={t} confirm={confirm} api={api}
      />
    );
  }
    ...
    <div className="hl-page">
      ...
      <>
          <div className="hl-week-strip">
            <button className="hl-icon-btn" onClick={() => { const d = new Date(anchor + "T00:00:00"); d.setDate(d.getDate() - 7); setAnchor(isoDate(d)); }}><ChevronLeft size={14} /></button>
            {days.map((iso, i) => (
            ...
            <button className="hl-icon-btn" onClick={() => { const d = new Date(anchor + "T00:00:00"); d.setDate(d.getDate() + 7); setAnchor(isoDate(d)); }}><ChevronRight size={14} /></button>
          </div>
...
function SolveHomework({ hw, mode = "new", user, submissions, setSubmissions, reattempts, setReattempts, showToast, onDone, t, confirm, api }) {
    ...
    return (
      <div className="hl-page">
        <div className="hl-result-card">
          ...
          <div className="hl-result-list">
            {hw.questions.map((q) => {
              ...
              return (
                <div className={`hl-result-row ${d.ungraded ? "" : d.correct ? "ok" : "bad"}`} key={q.id}>
                  {d.ungraded ? <ImageIcon size={14} /> : d.correct ? <Check size={14} /> : <X size={14} />}
                  <span className="q">{q.type === "vertical" ? `${toDevanagariDigits(q.a)} ${OP_SYMBOLS[q.op]} ${toDevanagariDigits(q.b)}` : q.type === "photo" ? t("photoQuestionLabel") : q.text}</span>
                  <span className="a">
                    {d.ungraded ? t("photoAnswerNote") : <>{t("yourAnswer")} {vertDisplay(q, d.given) || "—"}{!d.correct && <> · {t("correctLabel")} {vertDisplay(q, q.answer)}</>}</>}
                  </span>
                </div>
              );
            })}
          ...
          {!isReattempt && api && (
            <div className="hl-embedded-section hl-today-lb">
              <h3 className="hl-subhead">{t("todaysLeaderboard")}</h3>
              <Leaderboard api={api} highlightId={user.id} lockClass={user.className} embedded period="daily" hideControls />
            </div>
          )}
            ...
            {q.type === "vertical" ? (
              <>
                ...
                <div className="hl-vertical-problem">
                  <div className="hl-vert-num">{toDevanagariDigits(q.a)}</div>
                  <div className="hl-vert-op-row"><span className="hl-vert-op">{OP_SYMBOLS[q.op]}</span><span className="hl-vert-num">{toDevanagariDigits(q.b)}</span></div>
                  <div className="hl-vert-line" />
                  <input className="hl-vert-answer" placeholder={t("vAnswerPlaceholder")} value={answers[q.id] || ""} onChange={(e) => setAnswers((a) => ({ ...a, [q.id]: toDevanagariDigits(e.target.value) }))} />
                </div>
              </>
            ) : q.type === "mcq" ? (
              <>
                ...
                <div className="hl-q-photo">
                  {answerImages[q.id] ? (
                    ...
                    <label className="hl-btn ghost sm hl-upload-btn">
                      <ImageIcon size={14} /> {t("takePhoto")}
                      {/* capture="environment" opens the phone's camera app directly instead of a file/gallery picker */}
                      <input type="file" accept="image/*" capture="environment" hidden onChange={(e) => { setPhotoAnswer(q.id, e.target.files && e.target.files[0]); e.target.value = ""; }} />
                    </label>
                  )}
                </div>
              </>
...
// ---------- LEADERBOARD (shared) ----------
function Leaderboard({ api, highlightId, lockClass, embedded, period: fixedPeriod, hideControls }) {
  const { users, submissions, classes, t } = api;
  const [clsFilter, setClsFilter] = useState(lockClass || "all");
  const [period, setPeriod] = useState(fixedPeriod || "weekly");
  const effectiveFilter = lockClass || clsFilter;
  const effectivePeriod = fixedPeriod || period;
  ...
  const periodStartMs = useMemo(() => {
    ...
    return new Date(now.getFullYear(), now.getMonth(), now.getDate() - dow, 0, 0, 0, 0).getTime();
  }, [effectivePeriod]);
  ...
  const rows = useMemo(() => {
    ...
    return students.map(([id, u]) => {
      const subs = submissions.filter((s) => s.studentId === id && (s.submittedAt || 0) >= periodStartMs);
  ...
  }, [users, submissions, effectiveFilter, periodStartMs]);
  ...
  const body = (
    <>
      {!hideControls && (
        <div className="hl-segmented hl-lb-period">
          <button type="button" className={effectivePeriod === "daily" ? "active" : ""} onClick={() => setPeriod("daily")}>{t("lbDaily")}</button>
          <button type="button" className={effectivePeriod === "weekly" ? "active" : ""} onClick={() => setPeriod("weekly")}>{t("lbWeekly")}</button>
          <button type="button" className={effectivePeriod === "monthly" ? "active" : ""} onClick={() => setPeriod("monthly")}>{t("lbMonthly")}</button>
        </div>
      )}
      {!lockClass && !hideControls && (
        <label className="hl-field hl-jump">
...
// ---------- NOTES (photos & PDFs) ----------
// Photos taken on a phone camera can easily be 4-8MB. Storing that as raw
// base64 (in every homework record, synced live to every device) is what was
// causing a white/blank screen after uploading a photo — the payload got too
// large for the realtime sync and/or froze the page while stringifying it.
// Downscaling + re-encoding as JPEG here keeps each photo to a few hundred KB.
  ...
  return new Promise((resolve, reject) => {
    ...
    img.onload = () => {
      ...
      try {
        ...
        // canvas can fail in some browsers/edge cases — fall back to the raw file
        readFileAsDataUrl(file).then(resolve, reject);
      }
    };
    img.onerror = () => { cleanup(); readFileAsDataUrl(file).then(resolve, reject); };
    img.src = objectUrl;
  });
}
    ...
    <div className="hl-page">
      ...
      {isTeacher && (
        <div className="hl-card">
          ...
          {previewId && (
            <div className="hl-video-preview">
              <img src={ytThumb(previewId)} alt="" />
              <span>{title.trim() || t("videosTitle")}</span>
            </div>
          )}
      ...
      <div className="hl-video-list">
        ...
        {videos.map((v, i) => (
          <div className="hl-video-item" key={v.id}>
            <button type="button" className="hl-video-thumb" onClick={() => setPlaying(v)}>
              <img src={ytThumb(v.youtubeId)} alt="" loading="lazy" />
              <span className="hl-video-play-badge"><Play size={16} fill="#FFFFFF" /></span>
            </button>
            <div className="hl-video-info">
              <div className="hl-hw-title">{v.title}</div>
              {isTeacher && (
                <div className="hl-row-actions">
                  <button className="hl-icon-btn sm" disabled={i === 0} onClick={() => move(i, -1)}><ArrowUp size={13} /></button>
                  <button className="hl-icon-btn sm" disabled={i === videos.length - 1} onClick={() => move(i, 1)}><ArrowDown size={13} /></button>
                  <button className="hl-icon-btn sm danger" onClick={() => removeVideo(v)}><Trash2 size={13} /></button>
                </div>
              )}
            </div>
          </div>
      ...
      {playing && <VideoPlayerModal video={playing} onClose={() => setPlaying(null)} t={t} />}
    </div>
  );
}

function VideoPlayerModal({ video, onClose, t }) {
  return (
    <div className="hl-modal-overlay" onClick={onClose}>
      <div className="hl-video-modal-card" onClick={(e) => e.stopPropagation()}>
        <div className="hl-zoom-toolbar">
          <span className="hl-note-name">{video.title}</span>
          <button className="hl-icon-btn" onClick={onClose} title={t("close")}><X size={16} /></button>
        </div>
...
function Style() {
  return (
    <style>{`
      ...
      /* week strip */
      .hl-week-strip { display: flex; align-items: center; gap: 2px; background: var(--card); border: 1px solid var(--border); border-radius: 14px; padding: 6px 4px; margin-bottom: 14px; overflow-x: hidden; }
      .hl-week-strip .hl-icon-btn { flex: 0 0 auto; width: 26px; height: 26px; min-width: 26px; padding: 0; }
      .hl-day { flex: 1 1 0; min-width: 0; width: 0; border: 1px solid transparent; background: transparent; border-radius: 9px; padding: 6px 1px; display: flex; flex-direction: column; align-items: center; gap: 1px; cursor: pointer; position: relative; min-height: 46px; }
      .hl-day-name { font-size: 8.5px; color: var(--ink-soft); font-weight: 700; text-transform: uppercase; }
      ...
      .hl-lb-period { max-width: 320px; }
      .hl-today-lb { margin-top: 18px; text-align: left; }
      .hl-today-lb .hl-subhead { margin-top: 0; }
      ...
      .hl-vbuilder { padding-top: 4px; margin-top: 2px; }
      ...
      .hl-vb-preview { margin-top: 8px; font-size: 15px; font-weight: 700; color: var(--board-dark); background: var(--paper); border: 1px dashed var(--border); border-radius: 9px; padding: 8px 12px; width: fit-content; }
      .hl-q-vert-mark { display: block; margin-top: 2px; color: var(--gold-dark); }

      /* question-type tabs (reduce clutter in homework builder) */
      .hl-qtype-tabs { margin: 2px 0 4px; }
      .hl-qtype-add { display: flex; flex-direction: column; gap: 6px; align-items: flex-start; padding-top: 2px; }
      .hl-qtype-hint { font-size: 12.5px; color: var(--ink-soft); margin: 0; }
      .hl-qlist-toggle { display: flex; align-items: center; gap: 6px; border: none; background: none; padding: 10px 0 6px; margin-top: 6px; border-top: 1px dashed var(--border); font-size: 13px; font-weight: 700; color: var(--ink-soft); cursor: pointer; width: 100%; }
      ...
      /* videos */
      .hl-video-list { display: flex; flex-direction: column; gap: 8px; }
      .hl-video-item { display: flex; align-items: center; gap: 10px; background: var(--card); border: 1px solid var(--border); border-radius: 12px; padding: 8px; }
      .hl-video-thumb { position: relative; flex: 0 0 112px; width: 112px; aspect-ratio: 16 / 9; border-radius: 8px; overflow: hidden; background: #000; border: none; padding: 0; cursor: pointer; }
      .hl-video-thumb img { width: 100%; height: 100%; object-fit: cover; display: block; }
      .hl-video-play-badge { position: absolute; inset: 0; display: flex; align-items: center; justify-content: center; background: rgba(0,0,0,0.28); }
      .hl-video-play-badge svg { filter: drop-shadow(0 1px 2px rgba(0,0,0,0.5)); }
      .hl-video-info { flex: 1; min-width: 0; }
      .hl-video-info .hl-hw-title { font-size: 13.5px; display: -webkit-box; -webkit-line-clamp: 2; -webkit-box-orient: vertical; overflow: hidden; margin-bottom: 6px; }
      .hl-video-preview { display: flex; align-items: center; gap: 10px; margin: -6px 0 12px; font-size: 13px; color: var(--ink-soft); }
      .hl-video-preview img { width: 72px; aspect-ratio: 16 / 9; object-fit: cover; border-radius: 7px; border: 1px solid var(--border); flex-shrink: 0; }
      ...
      .hl-video-modal-card { background: var(--card); border-radius: 14px; width: 100%; max-width: 720px; overflow: hidden; }
      ...
      @media (max-width: 400px) {
        ...
        .hl-week-strip { gap: 1px; padding: 5px 2px; }
        .hl-week-strip .hl-icon-btn { width: 22px; height: 22px; min-width: 22px; }
        .hl-day { padding: 5px 0; min-height: 42px; }
        .hl-day-name { font-size: 7.5px; }
        .hl-day-num { font-size: 13px; }
      }
    `}</style>
  );
}
